...
class Mailchimp_Reports
{
	 ...
	 */
	public function abuse($cid, $opts=[])
	{
...
messages over 30 days old are subject to being removed
	 ...
	 *         - message string the entire bounce message received
	 */
	public function bounceMessages($cid, $opts=[])
	{
		$_params = ["cid" => $cid, "opts" => $opts];
	 ...
	 */
	public function clickDetail($cid, $tid, $opts=[])
	{
	 ...
	 */
	public function ecommOrders($cid, $opts=[])
	{
		$_params = ["cid" => $cid, "opts" => $opts];
...
<strong>not</strong> this, especially for large campaigns.
	 ...
	 */
	public function notOpened($cid, $opts=[])
	{
		$_params = ["cid" => $cid, "opts" => $opts];
	 ...
	 */
	public function opened($cid, $opts=[])
	{
		$_params = ["cid" => $cid, "opts" => $opts];
...
or campaignEmailStatsAIMAll() and generate any additional stats they require.
	 ...
	 */
	public function sentTo($cid, $opts=[])
	{
		$_params = ["cid" => $cid, "opts" => $opts];
	 ...
	 */
	public function share($cid, $opts=[])
	{
		$_params = ["cid" => $cid, "opts" => $opts];
	 ...
	 */
	public function unsubscribes($cid, $opts=[])
	{
		$_params = ["cid" => $cid, "opts" => $opts];
